...
function coinGeckoApi() {
  ...
  fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&ids=ripple&sparkline=true&price_change_percentage=1h%2C24h%2C7d%2C14d%2C30d%2C1y", myInit)
  ...
  .then(data => {
    ...
    liChange24h[0].textContent = data[0].price_change_24h.toFixed(4) + '€ /24h';
    ...
    liHigh24h[0].textContent = '€' + data[0].high_24h.toFixed(4);
    liLow24h[0].textContent = '€' + data[0].low_24h.toFixed(4);
...
function coinbaseApi() {
    ...
    fetch('https://api.pro.coinbase.com/products/XRP-EUR/ticker')
        .then(function (response) {
            return response.json();
        }).then(function (data) {
            coin = data;

            liPrice[0].textContent = '€' + parseFloat(coin.price).toFixed(4);
            siteTitle[0].textContent = 'XRP €' + parseFloat(coin.price).toFixed(4) + ' - Crypto Coins Now';
